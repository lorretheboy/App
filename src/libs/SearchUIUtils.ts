...
type GetSectionsParams = {
    ...
    personalCardList?: OnyxEntry<OnyxTypes.CardList>;
...
function getReportSections({
    ...
    for (const key of orderedKeys) {
        if (isReportEntry(key) && (data[key].type === CONST.REPORT.TYPE.IOU || data[key].type === CONST.REPORT.TYPE.EXPENSE || data[key].type === CONST.REPORT.TYPE.INVOICE)) {
            ...
            const actions = Object.values(data[`${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${reportItem.reportID}`] ?? {});
...
function getCardSections(
    ...
    for (const key in data) {
        if (isGroupEntry(key)) {
            ...
            const cardForPersonalCheck = personalCardList?.[cardGroup.cardID] ?? card;
            cardSections[key] = {
                ...
                formattedCardName,
                formattedFeedName:
                    cardForPersonalCheck && isPersonalCard(cardForPersonalCheck)
...
function getSections({
    ...
    if (groupBy) {
        ...
        switch (groupBy) {
            ...
            case CONST.SEARCH.GROUP_BY.CARD:
                return getCardSections(data, queryJSON, translate, cardFeeds, customCardNames, cardList, personalCardList);
            case CONST.SEARCH.GROUP_BY.WITHDRAWAL_ID:
