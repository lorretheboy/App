 ...
 *
 * Intentionally excluded: What's new, Sign out, Save the world, and per-domain pages (the latter
 * require a domain account ID).
 */
...
const NAVIGATION_OPTIONS: NavigationOption[] = [
    // Account / Settings pages (What's new, Sign out, Save the world are intentionally omitted)
 ...
 * can actually see it on the Spend page.
 */
...
/** Distinct icon names referenced by the static Account config, used to pre-resolve icons via the lazy icon hook. */
    ...
    return options
        ...
        .map(
            ({option, title}): SearchQueryItem => ({
                text: translate('search.goTo', title),
...
/**
 * Returns the navigation rows for Account sub-pages that match the typed query. Each row carries the
 * Account tab label in `rightText` so the user can tell which top-level tab the page lives under.
 */
 ...
 * the Spend page with that tab's search query and carries the Spend tab label in `rightText`.
 */
 ...
 * produced per visible workspace and enabled page; the workspace name is carried in `rightText` and
 * the workspace avatar in `rightAvatar` so the user can tell which workspace each row refers to.
 */
